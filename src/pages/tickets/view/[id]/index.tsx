...
import { useTicketFindFirst } from 'lib/roq';
...
function TicketViewPage() {
  ...
  const queryParams = useMemo<{ include: { lottery: boolean; user: boolean } }>(
    () =>
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem label="Is Winner" text={data?.is_winner} />
                ...
                {hasAccess('lottery', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Lottery"
                    text={
                      <Link as={NextLink} href={`/lotteries/view/${data?.lottery?.id}`}>
                        {data?.lottery?.name}
                      </Link>
                    }
                  />
