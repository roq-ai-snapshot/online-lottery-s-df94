...
import { useRoqClient, useVendorFindManyWithCount } from 'lib/roq';
...
export function VendorListPage(props: VendorListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useVendorFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.vendor.delete({ where: { id } });
      await mutate();
    } catch (error) {
