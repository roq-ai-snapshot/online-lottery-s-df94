...
import { useRoqClient, useLotteryFindFirst } from 'lib/roq';
...
function LotteryEditPage() {
  ...
  const { data, error, isLoading, mutate } = useLotteryFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: LotteryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.lottery.update({
        data: values as RoqTypes.lottery,
        where: {
          id,
        },
      });
